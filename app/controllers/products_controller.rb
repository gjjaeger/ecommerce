class ProductsController < ApplicationController
  ...
  def index
    ...
    bsellerjewelery=Product.where("(category_id = ? AND featured = ? AND stock > ?)", "2", true, 0).limit(3)
    restjewelery=Product.where("(category_id = ? AND stock > ?)", "2", 0).limit(8-bsellerjewelery.length())
